...
{strip}
	...
	{if $FIELD_MODEL->getUIType() eq '71'}
		<div class="input-group">
			{if $SYMBOL_PLACEMENT neq '1.0$'}
				<span class="input-group-append"><span class="input-group-text js-currency" data-js="text"> {$USER_MODEL->get('currency_symbol')}</span></span>
			{/if}
			<input id="{$MODULE}_editView_fieldName_{$FIELD_NAME}" type="numeric" title="{\App\Language::translate($FIELD_MODEL->getFieldLabel(), $MODULE)}" class="currencyField form-control {if $SYMBOL_PLACEMENT eq '1.0$'} textAlignRight {/if}" name="{$FIELD_MODEL->getFieldName()}"
				   data-fieldinfo='{$FIELD_INFO}' value="{if $FIELD_NAME eq 'reg_recu' && $REG_RECU}{$REG_RECU}
				   {elseif $FIELD_NAME eq 'reg_a_recevoir' && $REG_A_RECEVOIR}{$REG_A_RECEVOIR}
				   {elseif $FIELD_NAME eq 'montant' && !$FIELD_VALUE }{$REG_A_RECEVOIR - $REG_RECU}{else}{$FIELD_VALUE}{/if}"
	...
	{else if ($FIELD_MODEL->getUIType() eq '72') && ($FIELD_MODEL->getName() eq 'unit_price')}
		<div class="input-group">
			{if $SYMBOL_PLACEMENT neq '1.0$'}
				<span class="input-group-append row"><span class="input-group-text js-currency" data-js="text">{$BASE_CURRENCY_SYMBOL}</span></span>
			{/if}
	...
	{else}
		<div class="input-group">
			<div class="row">
				<span class="col-md-1 input-group-append"><span class="input-group-text row js-currency" data-js="text">{$USER_MODEL->get('currency_symbol')}</span></span>
					{assign var="DISPLAY_FIELD_VALUE" value=$FIELD_VALUE}
