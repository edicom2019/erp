	...
	<div class="modal-body">
		...
		<div class="btn-elements text-center">
			{foreach item=TEMPLATE from=$TEMPLATES}
				...
				<button class="btn btn-light js-genetate-button border" data-js="click" data-id="{$TEMPLATE->getId()}" data-name="{$RELATED_MODEL->getName()}" data-url="{$RELATED_MODEL->getCreateRecordUrl()|cat:"&reference_id=$RECORD"}&generateReglement=1">
					<span class="userIcon-{$TEMPLATE->getRelatedName()}"></span>
					&nbsp;Generer Reglement
				</button>
			{/foreach}
