...
class Reglements_Edit_View extends Vtiger_Edit_View
{
	/**
	...
	public function process(\App\Request $request)
	{
    	...
    	$reg_recu=(new App\Db\Query())->select(['sum(r.montant) as montants'])->from('u_yf_reglements r')
            ->innerJoin('u_yf_finvoice finv', 'finv.num_ordre = r.factu and finv.finvoiceid='.$request->getInteger('reference_id'))
            ->innerJoin('vtiger_crmentity ent', 'ent.crmid = r.reglementsid and ent.deleted=0')
            ->one();

    	$reg_a_recevoir=(new App\Db\Query())->select(['sum_gross','finvoiceid'])->from('u_yf_finvoice')->where(['finvoiceid' => $request->getInteger('reference_id')])->one();
		...
		$viewer->assign('REG_RECU', $reg_recu["montants"]);
		$viewer->assign('REG_A_RECEVOIR', $reg_a_recevoir['sum_gross']);
		$viewer->assign('TEST', $reg_a_recevoir['finvoiceid']);
